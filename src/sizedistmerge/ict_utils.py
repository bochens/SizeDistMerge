...
def number_to_surface_area_spectrum(mids_nm, dNdlogDp, frac_sigma, unit="um"):
    ...
    if D_nm.ndim != 1 or y.ndim != 1 or D_nm.size != y.size:
        raise ValueError("mids_nm and dNdlogDp must be 1D arrays with the same length")
    if np.any(~np.isfinite(D_nm)) or np.any(D_nm <= 0):
        raise ValueError("mids_nm must be finite and > 0")
    if np.any(np.isinf(y)):
        raise ValueError("dNdlogDp must not contain infinite values")
    frac_sigma = np.asarray(frac_sigma, float)
    if frac_sigma.ndim > 1:
        raise ValueError("frac_sigma must be scalar or 1D")
    if frac_sigma.ndim == 1 and frac_sigma.size not in (1, y.size):
        raise ValueError("frac_sigma must be scalar or match dNdlogDp length")
    if np.any(~np.isfinite(frac_sigma)) or np.any(frac_sigma < 0):
        raise ValueError("frac_sigma must be finite and >= 0")
