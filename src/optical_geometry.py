...
def load_optical_setup(name_or_path):
    ...
    if requested.suffix == '.toml':
        ...
        if not name.replace('_', '').isalnum():
            raise ValueError('Use a built-in TOML stem or an explicit .toml path')
...
class OpticalSetup:
    ...
    def __post_init__(self):
        ...
        if self.response_channel is not None and self.response_channel not in names:
            raise ValueError("response_channel must name a configured detector")
...
# This identifies the calculation method, not an instrument preset.
OPTICAL_MODEL_VERSION = "solid-angle-polarized-cones-v1"


def optical_setup_from_lut_metadata(attrs):
    """Recover the setup recorded when the LUT was calculated.

    Never substitute today's TOML for missing historical metadata. A setup
    edit must not silently change the geometry used to describe an older LUT.
    """
    if "optical_setup" not in attrs:
        raise ValueError("LUT lacks its full optical_setup; rebuild it or explicitly migrate its verified metadata")
